...
if ( !empty($_POST)) { // if $_POST filled then process the form
	...
	# initialize/validate (same as file: iss_issue_create.php)
	...
	if ($valid) { // if valid user input update the database
		...
		$sql = "UPDATE iss_issues  set short_description = ?, long_description = ?, open_date = ?, close_date = ?, priority = ? WHERE id = ?";
		$q = $pdo->prepare($sql);
		$q->execute(array($shortDescription,$longDescription,$openDate,$closeDate,$priority,$id));
		Database::disconnect();
...
<body>
    <div class="container">	
		<div class="span10 offset1">
			...
			<div class="row">
				<h3>Update Issue Details</h3>
			</div>
	
			<form class="form-horizontal" action="iss_issue_update.php?id=<?php echo $id?>" method="post">
